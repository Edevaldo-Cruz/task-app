...
const scheduleTaskNotification = async (task: Tarefa) => {
  ...
  const month = parseInt(dateParts[1]) - 1; (0-11)
...
export default function TabOneScreen() {
    ...
    const handleCreateTask = async () => {
      ...
      if (data < dataAtual ||
        (data === dataAtual &&
          (horaSelecionada < agora.getHours() ||
            (horaSelecionada === agora.getHours() && minutoSelecionado < agora.getMinutes())))) {
      ...
      `;
        ...
        const result = db.getAllSync<{ id: number }>("SELECT last_insert_rowid() as id");
    ...
    useEffect(() => {
      ...
      const requestPermissions = async () => {
        ...
        if (status !== 'granted') {
          Alert.alert('Permissão negada', 'As notificações não funcionarão sem permissão');
        }
...
>
  <Text style={{ color: 'white' }}>Testar Notificação</Text>
</TouchableOpacity> */}
